    ...
    True -> {
      ...
      case string.split_once(without_first_dash, "---") {
        Ok(#(frontmatter, body)) -> {
          ...
          let parsed = list.fold(lines, #(None, None), fn(acc, line) {
          ...
          case parsed {
            #(Some(name), Some(description)) -> {
              ...
              Ok(Skill(
                ...
                rules: [],
                facts: [Datom(name, "skill/prompt", clean_body)]
              ))
            }
...
pub fn load_skills_from_dir(dir_path: String) -> Result(List(Skill), String) {
  case simplifile.read_directory(dir_path) {
    Ok(children) -> {
      let skills = list.filter_map(children, fn(child) {
    ...
    Error(err) -> Error("Could not read directory " <> dir_path <> ": " <> string.inspect(err))
  }
}
