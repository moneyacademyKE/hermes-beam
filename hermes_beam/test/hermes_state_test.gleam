...
pub fn state_db_workflow_test() {
  ...
  // 3. Create a session
  let assert Ok(Nil) = hermes_state.create_session(
    conn,
    "session-123",
    "test-suite",
    "gpt-4o",
  ...
  // 4. Update CWD
  let assert Ok(Nil) = hermes_state.update_session_cwd(conn, "session-123", "/workspace/test")
  
  // 5. End session
  let assert Ok(Nil) = hermes_state.end_session(conn, "session-123", "completed", 1_717_005_000.0)
  ...
  let assert Ok(matches) = hermes_state.search_messages(conn, "code_execution_tool")
  ...
  let assert "Hello agent, please find the bug in code_execution_tool.py" = first_match.content
